...
class _AddSuppliersFormState extends State<_AddSuppliersForm> {
  ...
  Supplier _prepareNewTaxes() {
    final supplier = _suppliers.first;

    // Append supplier-code to contact person names
  ...
  void _onSubmit() {
    ...
    final newSupplier = _prepareNewTaxes();
    context.read<SupplierBloc>().add(AddSetup<Supplier>(data: newSupplier));
