...
import 'package:assign_erp/features/pos_system/data/models/pos_print/pos_report_builder.dart';
import 'package:assign_erp/features/pos_system/data/models/pos_sale_model.dart';
import 'package:collection/collection.dart';
import 'package:pdf/pdf.dart';
import 'package:printing/printing.dart';

/// Print POS Sales Report [POSReportPrinter]
class POSReportPrinter {
  final List<POSSale> sales;
  final String storeNumber;

  POSReportPrinter({required this.sales, required this.storeNumber});
  ...
  List<ReportItem> _buildLineItems() {
    // Group sales by date
  ...
  Future<Uint8List> _generatePdf({
    ...
    // Aggregate data for each group
    final salesReports = _buildLineItems();

    final salesReport = POSReportBuilder(
    ...
    return await salesReport.build(format);
  }
